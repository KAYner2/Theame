...
export default function About() {
  ...
  const values = [
    { icon: <Heart className="w-8 h-8" />, title: 'Любовь к цветам', description: 'Каждый букет создается с любовью и вниманием к деталям' },
    { icon: <Award className="w-8 h-8" />, title: 'Качество', description: 'Используем только свежие цветы от проверенных поставщиков' },
    { icon: <Users className="w-8 h-8" />, title: 'Клиентоориентированность', description: 'Всегда ставим потребности клиентов на первое место' },
    { icon: <Clock className="w-8 h-8" />, title: 'Пунктуальность', description: 'Доставляем точно в срок, чтобы не испортить важный момент' },
  ];

  return (
    /* ЕДИНЫЙ песочный фон на всю страницу */
    <div className="min-h-screen bg-[#fff8ea]">
      {/* Hero без собственного фона/градиента */}
      <section className="py-16 sm:py-20">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl sm:text-5xl font-bold mb-4 text-[#819570]">The Áme</h1>
      ...
      <section className="py-16 sm:py-20">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-[#819570]">О нас</h2>
            ...
            {/* лёгкие карточки-статы, не меняют общий фон */}
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <Card key={item.title} className="border-0 shadow-soft rounded-xl bg-white/60">
      ...
      {/* Ценности — тоже без отдельного фона */}
      <section className="py-16 sm:py-20">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold mb-3 text-[#819570]">Наши ценности</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((value, idx) => (
              <Card
                key={idx}
                className="border-0 shadow-soft rounded-xl bg-white/60 transition-transform duration-300 hover:shadow-elegant hover:-translate-y-0.5"
              >
                <CardContent className="p-6 text-center">
