...
export const AdminPanel = () => {
  ...
  const [activeTab, setActiveTab] = useState<'categories' | 'products' | 'reviews' | 'hero-slides'>('categories');
    ...
    return (
      <div className="max-h-[80vh] overflow-y-auto">
        <form onSubmit={handleSubmit} className="space-y-4 pr-2">
          ...
          <div>
            <Label htmlFor="guarantee_info">Гарантия</Label>
            <Textarea
              id="guarantee_info"
              value={formData.guarantee_info}
              onChange={(e) => setFormData({ ...formData, guarantee_info: e.target.value })}
              rows={2}
            />
          </div>
          <div>
