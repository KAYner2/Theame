...
// ▲▲▲ VARIANT PRODUCTS: конец блока ▲▲▲
  ...
  // ▼▼▼ единый DnD-обработчик для вкладки "products" ▼▼▼
const handleUnifiedDragEnd = (event: DragEndEvent) => {
  const { active, over } = event;
  if (!over || active.id === over.id) return;

  const oldIndex = combinedItems.findIndex((x) => String(x.id) === String(active.id));
  const newIndex = combinedItems.findIndex((x) => String(x.id) === String(over.id));
  if (oldIndex === -1 || newIndex === -1) return;
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
  ...
  {productsLoading ? (
    ...
    <>
      ...
      >
        <SortableContext
          items={combinedItems.map((p: any) => String(p.id))}
