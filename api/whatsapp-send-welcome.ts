...
export default async function handler(req: Request): Promise<Response> {
  ...
  // ---- Изменённый текст приветствия ----
  const effectivePromo = promoCode || "WELCOME300";
  const hello =
    `Здравствуйте${name ? `, ${name}` : ""}! 🌸\n` +
    `Спасибо, что подписались на рассылку The Áme 🌿\n\n` +
    `Ваш персональный промокод для первого заказа:\n` +
    `✨ ${effectivePromo} ✨\n\n` +
    `Дарим 300₽ скидки на первый заказ!\n\n` +
    `Мы верим: букет от The Áme — это начало новой традиции 🤍`;
  // --------------------------------------
